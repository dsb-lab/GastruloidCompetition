...
for TIME in TIMES:
    for COND in CONDS:
        ...
        for f, file in enumerate(files):
            ...
            # From https://stackoverflow.com/a/12233959
            def butter_bandpass_filter_img(img, lowcut, highcut, fs, order=5):
                b, a = butter_bandpass(lowcut, highcut, fs, order=order)
                new_img = lfilter(b, a, img, axis=0)
                return new_img
            ...
            for ch, ch_name in enumerate(channel_names):
                new_img = butter_bandpass_filter_img(CT_F3.hyperstack[0,:,channel_names.index(ch_name)], 0.1*zres, zres/2.01, zres, order=6)
                CT_F3.hyperstack[0,:,channel_names.index(ch_name)] = new_img
                new_img = butter_bandpass_filter_img(CT_A12.hyperstack[0,:,channel_names.index(ch_name)], 0.1*zres, zres/2.01, zres, order=6)
                CT_A12.hyperstack[0,:,channel_names.index(ch_name)] = new_img
                new_img = butter_bandpass_filter_img(CT_A12.hyperstack[0,:,channel_names.index(ch_name)], 0.1*zres, zres/2.01, zres, order=6)
                CT_Casp3.hyperstack[0,:,channel_names.index(ch_name)] = new_img
            ...
            thf3 = np.mean(f3_casp3)+0.5*np.std(f3_casp3)
            tha12 = np.mean(a12_casp3)+0.5*np.std(a12_casp3)
