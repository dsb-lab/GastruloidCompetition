...
p_values = []
ks_results = []
for i in range(len(DISTS_F3_WT[0])):
    A = DISTS_F3_WT[0][i]
    B = DISTS_A12_WT[0][i]
    ks_stat, p_val = ks_2samp(A, B)
    p_values.append(p_val)
    ks_results.append((i, ks_stat, p_val))
    print(f"Experiment {i}: KS={ks_stat:.3f}, p={p_val:.3e}")
...
print(f"Fisher's method chi2={chi2_stat:.3f}, combined p={combined_p:.3e}")
...
med_A, ci_low_A, ci_up_A = bootstrap_median_ci(all_A)
med_B, ci_low_B, ci_up_B = bootstrap_median_ci(all_B)
print(f"\nPooled Medians:")
...
# -------------------------
stat, pval_ks = ks_2samp(all_A, all_B)
...
perm_pval = permutation_test(all_A, all_B)
